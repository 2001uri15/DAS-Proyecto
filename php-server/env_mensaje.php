...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        ...
        if ($destino === 'individual' && !empty($token_individual)) {
            ...
            $sql = "SELECT `tokenFCM` FROM `Xalarrazabal025_user_app` WHERE `tokenFCM` IS NOT NULL";
                    ...
                    <form method="post" action="env_mensaje.php">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="destino" id="destino_individual" value="individual" checked>
                                <label class="form-check-label" for="destino_individual">
                                    Dispositivo individual
                                </label>
                            </div>
                            <div class="form-check">
                                <input class="form-check-input" type="radio" name="destino" id="destino_todos" value="todos">
                        ...
                        <div class="mb-3 token-group" id="token_group">
                            ...
                            <select name="token_individual">
                                <?php
                                    $sql2="SELECT `nombre`, tokenFCM FROM `Xalarrazabal025_usuarios` INNER JOIN Xalarrazabal025_user_app on Xalarrazabal025_usuarios.id=Xalarrazabal025_user_app.idUser;";
                                    $result = $conn->query($sql2);
                                    while ($row = $result->fetch_assoc()) {
                                        echo "<option value='".$row["tokenFCM"]."'>".$row["nombre"]."</option>";
                                    }
                                ?>
                            </select>
                        </div>
